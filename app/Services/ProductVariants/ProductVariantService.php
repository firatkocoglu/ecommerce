...
class ProductVariantService
{
    public function listByProductId(int $productId, bool $onlyActive = true, bool $cacheActive = true): Collection
    {
        ...
        $int = filter_var($productId, FILTER_VALIDATE_INT, ['options' => ['min_range' => 1]]);
        if ($int === false) {
    ...
    public function findById(int $productId, int $variantId, bool $onlyActive = true, bool $cacheActive = true): ProductVariant
    {
        ...
        $intProduct = filter_var($productId, FILTER_VALIDATE_INT, ['options' => ['min_range' => 1]]);
        $intVariant = filter_var($variantId, FILTER_VALIDATE_INT, ['options' => ['min_range' => 1]]);
